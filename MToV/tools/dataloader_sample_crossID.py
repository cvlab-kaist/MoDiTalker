...
landmark_dir = "/media/data1/EDGE_30fps/cross_id/aligned_npy"
...
class EvalDataset(Dataset):
    def __init__(
        ...
        **super_kwargs,  # Additional arguments for the Dataset base class.
        ...
        classes = sorted(audio_id)
        class_to_idx = {cls_name: i for i, cls_name in enumerate(classes)}

        imgs = make_imagefolder_hdtf_dataset(path, [self.ref_id], nframes, class_to_idx, False)

        """ Set the length of the dataset for just one identity loading """
        print("audio_id_lst", audio_id)
        print("path", path)
    ...
    def __getitem__(self, index):
        ...
        ldmk_pth = os.path.join(os.path.dirname(self.path[0].replace(DATA_LOCATION, landmark_dir)), f'audio_{self.audio_id}')
        ldmk_pth = os.path.join(ldmk_pth, f'id_{self.ref_id}')
        land_vid = np.stack(
            [
...
def test():
    DATA_LOCATION = "/media/data/HDTF_preprocessed/30_frame_re/HDTF"
    num_frames = 16
    # test_dataset = EvalDataset(DATA_LOCATION, identity="WDA_MarciaFudge_000", total_frames=num_frames, train=False, resolution=256, nframes=16)
    test_dataset = EvalDataset(
        DATA_LOCATION,
        audio_id="RD_Radio2_000",
        ref_id="RD_Radio33_000",
        total_frames=num_frames,
        overlap=True,
        train=False,
        resolution=256,
    ...
    first = test_dataset.__getitem__(0)
    first = test_dataset.__getitem__(1)
    first = test_dataset.__getitem__(2)

    test_dataset_classes = test_dataset.classes  # 312

    # asdf = [test for test in test_dataset_classes if test in train_dataset_classes]
    # import pdb
