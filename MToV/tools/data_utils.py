...
def make_imageclip_hdtf_dataset(dir, identity_list, nframes, class_to_idx, vid_diverse_sampling):
    """
    TODO: add xflip
    """
    ...
    for target in identity_list:
        if os.path.isdir(os.path.join(dir, target)) == True:
            ...
            if nframes > 0 and vid_diverse_sampling:
                ...
                item_frames_2 = []
                item_frames_3 = []
                for fi in natsorted(glob.glob(os.path.join(subfolder_path, "*.jpg"))):
                    if is_image_file(fi):
                        file_name = fi
                        file_path = file_name
...
def make_imagefolder_hdtf_dataset(dir, identity_list, nframes, class_to_idx, vid_diverse_sampling):
    """
    dir: /media/data/HDTF_preprocessed/30_frame_re/HDTF/
    identity_list: [WRA_GregWalden1_001, WRA_MichaelSteele_001 ,,,]
    ...
    for target in identity_list:
        ...
        if os.path.isdir(os.path.join(dir, target)) == True:
            n_video += 1
            subfolder_path = os.path.join(dir, target)  # /media/data/HDTF_preprocessed/30_frame_re/HDTF/WRA_GregWalden1_001
            ...
            if valid == True:
                dataset_list.append((subfolder_path, count))

    return dataset_list


def make_imagefolder_crema_dataset(dir, identity_list, nframes, class_to_idx, vid_diverse_sampling):
    """
    dir: /media/data/HDTF_preprocessed/30_frame_re/HDTF/
    identity_list: [WRA_GregWalden1_001, WRA_MichaelSteele_001 ,,,]
    ...
    for target in identity_list:
        ...
        if os.path.isdir(os.path.join(dir, target)) == True:
            ...
            for fi in natsorted(glob.glob(os.path.join(subfolder_path, "*.jpg"), recursive=True)):
                if is_image_file(fi):
                    valid = True
                    count += 1
                else:
            ...
            if valid == True:
                dataset_list.append((os.path.join(subfolder_path), count))
