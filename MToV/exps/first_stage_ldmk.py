...
from tools.trainer import first_stage_train, first_stage_x_l_train
...
def first_stage_ldmk(rank, args):
    ...
    if rank == 0:
        fn = file_name(args)
        logger = Logger(fn, logdir=args.log_dir)
        logger.log(args)
    ...
    contain_contour = args.typetype != "ldmk_wo_contour"
    train_loader, test_loader, total_vid = get_loaders(
        rank,
        args.data,
        args.res,
        args.timesteps,
    ...
    model = ViTAutoencoder(args.embed_dim, args.ddconfig).to(device)
    criterion = LPIPSWithDiscriminator(disc_start=args.lossconfig.params.disc_start, timesteps=args.ddconfig.timesteps).to(device)

    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, betas=(0.5, 0.9))
    d_opt = torch.optim.AdamW(
        list(criterion.discriminator_2d.parameters()) + list(criterion.discriminator_3d.parameters()), lr=args.lr, betas=(0.5, 0.9)
    )

    if args.resume and rank == 0:
        model_ckpt = torch.load(args.first_model)
    ...
    if args.n_gpus > 1:
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[device], broadcast_buffers=False, find_unused_parameters=False)
        criterion = torch.nn.parallel.DistributedDataParallel(criterion, device_ids=[device], broadcast_buffers=False, find_unused_parameters=False)
    ...
    if args.typetype == "x":
        ...
        print("x_l")
        first_stage_x_l_train(rank, model, opt, d_opt, criterion, train_loader, test_loader, args.first_model, fp, logger)
