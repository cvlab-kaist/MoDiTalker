...
SAVING_DIR = path.abspath(__file__).split("/")[-2] 
class Logger(object):
    """Reference: https://gist.github.com/gyglim/1f8dfb1b5c82627ae3efcfbbadb9f514"""

    def __init__(self, fn, logdir, ask=True):
        
        self.saving_pth = f"{logdir}/{SAVING_DIR}/"
        
        if not os.path.exists(os.path.dirname(self.saving_pth)):
            os.makedirs(os.path.dirname(self.saving_pth), exist_ok=True)
        
        if not os.path.exists(self.saving_pth):
            os.makedirs(self.saving_pth, exist_ok=True)
    ...
    def _make_dir(self, fn):
        logdir = os.path.join(self.saving_pth, fn)
        return logdir
